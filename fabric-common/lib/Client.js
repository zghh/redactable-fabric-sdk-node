...
const RedactEventer = require('./RedactEventer');
...
const Client = class {
     ...
     * @param {string} [mspid] - Optional. The MSP id
     * @returns {Eventer} The RedactEventer instance.
     */
    ...
    /**
     * Returns a {@link RedactEventer} instance with the given name.
     * Will return a new instance. Does not check for existing instances
     * and does not keep a reference to this instance.
     *
     * @param {string} name - The name of the Eventer.
     * @param {string} [mspid] - Optional. The MSP id
     * @returns {RedactEventer} The Eventer instance.
     */
    newRedactEventer(name = checkParameter('name'), mspid) {
        const method = `newRedactEventer: ${this.name}`;
        logger.debug('%s start name:%s', method, name);

        const eventer = new RedactEventer(name, this, mspid);

        logger.debug('%s return new RedactEventer name:%s', method, name);
        return eventer;
    }
