...
function decodeBlockHeader(blockHeaderProto) {
    ...
    block_header.randomness = blockHeaderProto.randomness;
    block_header.redactor = blockHeaderProto.redactor;
...
function decodeBlockDataEnvelope(envelopeProto) {
    ...
    switch (envelope.payload.header.channel_header.type) {
        ...
        case 3:
            ...
            break;
        case 10:
            envelope.payload.data = decodeRedactTransaction(payloadProto.data);
            break;
        case 11:
...
function decodeRedactTransaction(dataBuf) {
    const data = {};
    try {
        const redactTransactionProto = fabproto6.common.RedactTransaction.decode(dataBuf);
        data.timestamp = redactTransactionProto.timestamp;
        data.current_height = redactTransactionProto.current_height;
        data.block_height = redactTransactionProto.block_height;
        data.transaction = [];
...
function decodeRedactBlock(dataBuf) {
    const data = {};
    try {
        const redactBlockProto = fabproto6.common.RedactBlock.decode(dataBuf);
        data.timestamp = redactBlockProto.timestamp;
        data.current_height = redactBlockProto.current_height;
        data.block_height = redactBlockProto.block_height;
        data.block = BlockDecoder.decodeBlock(redactBlockProto.block.block);
