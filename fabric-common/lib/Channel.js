...
const RedactEventService = require('./RedactEventService.js');
...
const Channel = class {
    ...
    /**
     * Returns a new {@link RedactEventService} instance
     *
     * @param {string} name - The name of this redact event service.
     */
    newRedactEventService(name = checkParameter('name')) {
        const method = `newRedactEventService[${this.name}]`;
        logger.debug(`${method} - start`);

        const eventService = new RedactEventService(name, this);
