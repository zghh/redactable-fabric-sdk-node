...
import { RedactEventServiceManager } from "./impl/event/redacteventservicemanager";
...
import {
  ...
  ListenerSession,
  removeListener,
} from "./impl/event/listenersession";
import { SharedBlockListenerSession } from "./impl/event/sharedblocklistenersession";
import { RedactMessageListenerSession } from "./impl/event/redactlistenersession";
import { QueryHandler } from "./impl/query/queryhandler";
import { notNullish } from "./impl/gatewayutils";
import * as Logger from "./logger";
import { RedactMessageEventSource } from "./impl/event/redacteventsource";

const logger = Logger.getLogger("Network");

async function listenerOptionsWithDefaults(
  options: ListenerOptions
): Promise<ListenerOptions> {
...
export class NetworkImpl implements Network {
  ...
  private redacteventServiceManager: RedactEventServiceManager;
  ...
  private readonly redactMessageListeners = new Map<
    RedactMessageListener,
    ListenerSession
  >();
  ...
  private readonly redactMessageEventSource: RedactMessageEventSource;
  ...
  constructor(gateway: Gateway, channel: Channel) {
    ...
    this.redacteventServiceManager = new RedactEventServiceManager(this);
    this.realtimeFilteredBlockEventSource = new BlockEventSource(
      this.eventServiceManager,
      { type: "filtered" }
    );
    this.realtimeFullBlockEventSource = new BlockEventSource(
      this.eventServiceManager,
      { type: "full" }
    );
    this.realtimePrivateBlockEventSource = new BlockEventSource(
      this.eventServiceManager,
      { type: "private" }
    );
    this.redactMessageEventSource = new RedactMessageEventSource(
      this.redacteventServiceManager
    );
  }
  ...
  getContract(chaincodeId: string, name = ""): Contract {
    ...
    if (!this.initialized) {
      throw new Error(
        "Unable to get contract as this network has failed to initialize"
      );
    }
  ...
  async addBlockListener(
    ...
    const sessionSupplier = async () =>
      await this.newBlockListenerSession(listener, options);
  ...
  async addRedactMessageListener(
    ...
    const sessionSupplier = async () =>
      await this.newRedactMessageListenerSession(listener);
    return await addListener(
      listener,
      this.redactMessageListeners,
      sessionSupplier
    );
  }

  removeRedactMessageListener(listener: RedactMessageListener): void {
    removeListener(listener, this.redactMessageListeners);
  }
   ...
   * @private
   */
  private async _initializeInternalChannel(
    ...
    if (options?.enabled) {
      ...
      logger.debug(
        "%s - user has not specified discovery targets, check channel and client",
        method
      );
      ...
      for (const peer of targets) {
        const discoverer = this.channel.client.newDiscoverer(
          peer.name,
          peer.mspid
        );
      ...
      this.discoveryService = this.channel.newDiscoveryService(
        this.channel.name
      );
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  private async newBlockListenerSession(
    listener: BlockListener,
    options: ListenerOptions
  ...
  private newSharedBlockListenerSession(
    ...
    if (type === "filtered") {
      return new SharedBlockListenerSession(
        listener,
      ...
      return new SharedBlockListenerSession(
        listener,
        this.realtimeFullBlockEventSource
      );
      ...
      throw new Error(
        `Unsupported event listener type: ${type as unknown as string}`
      );
    }
  }

  private newRedactMessageListenerSession(listener: RedactMessageListener) {
    return new RedactMessageListenerSession(
      listener,
      this.redactMessageEventSource
    );
  }
}
